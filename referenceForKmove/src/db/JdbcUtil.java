...
//		System.out.println(con); // null
		try {
			...
			//												lookup이 오브젝트타입이라 형변환해줌
			Context envCtx = (Context)initCtx.lookup("java:comp/env");
			// 3. <Resource> 태그 내의 name 속성 부분 가져오기(name 속성 값에 따라 변함)
			DataSource ds = (DataSource)envCtx.lookup("jdbc/MySQL");
//			System.out.println("JdbcUtill : " + ds);
			// 4. DataSource 객체(커넥션풀)로부터 Connection 객체 가져오기
			con = ds.getConnection();
//			System.out.println("ds.getConnection() : " + con); //JdbcUtill : org.apache.commons.dbcp2.BasicDataSource@a848253
